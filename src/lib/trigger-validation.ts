...
import { getEligibleMarkets } from './trigger-map';
...
function getEligibleMarketsLegacy(game: Record<string, any>): string[] {
  const TRIGGER_MAP = {
    'Over 0.5 Gols HT': (g: Record<string, any>) => {
    ...
    'Over 1.5 FT': (g: Record<string, any>) =>
      Number(g.exG ?? 0) >= 2.5,
    'Over 2.5 FT': (g: Record<string, any>) =>
      Number(g.exG ?? 0) >= 3.2 &&
      Math.max(Number(g.perc25H ?? 0), Number(g.perc25A ?? 0)) >= 50,
    'Ambas Marcam Sim': (g: Record<string, any>) => {
...
interface ComparisonResult {
  ...
  onlyLegacy: string[];
  onlyNew: string[];
  agreement: number; // percentual de concordância
}

function compareSystems(): ComparisonResult[] {
  const results: ComparisonResult[] = [];

  testGames.forEach(game => {
    const legacyMarkets = getEligibleMarketsLegacy(game);
    const newMarkets = getEligibleMarkets(game);
...
export { runValidation, compareSystems };
