...
exports.getEligibleMarkets = getEligibleMarkets;
exports.setNewTriggerEngine = setNewTriggerEngine;
exports.isMarketEligible = isMarketEligible;
// 🆕 Importar novo motor Poisson
const trigger_engine_1 = require("./trigger-engine");
const trigger_adapter_1 = require("./trigger-adapter");
...
exports.TRIGGER_MAP = {
    ...
    // ── CANTOS FT ────────────────────────────────────────────
    'Over 8.5 Cantos FT': (g) => Number(g.exC ?? 0) >= 9.5 ||
        (Number(g.cantFTH ?? 0) + Number(g.cantFTA ?? 0)) >= 10.0,
    'Over 9.5 Cantos FT': (g) => Number(g.exC ?? 0) >= 11.0 ||
        (Number(g.cantFTH ?? 0) + Number(g.cantFTA ?? 0)) >= 11.5,
    'Over 10.5 Cantos FT': (g) => Number(g.exC ?? 0) >= 12.5,
    // ── CANTOS HT ────────────────────────────────────────────
    ...
    '__consistente': (g) => {
        ...
        const cvGolsHT = Math.min(Number(g.cvGolsHT ?? 100), Number(g.cvGolsHT ?? 100));
        return cvGolsHT <= 55;
    },
};
// 🆕 Função wrapper que usa novo motor Poisson com fallback para sistema antigo
function getEligibleMarketsNew(game) {
    try {
        // Converter jogo para MatchInput
        const matchInput = (0, trigger_adapter_1.gameToMatchInput)(game);
        // Avaliar todos os mercados com novo motor
        const evaluations = (0, trigger_engine_1.evaluateAllMarkets)(matchInput);
...
function getEligibleMarketsLegacy(game) {
    const markets = Object.entries(exports.TRIGGER_MAP)
        .filter(([label, fn]) => !label.startsWith('__') && fn(game))
        .map(([label]) => label);
    return markets;
}
// 🆕 Função principal com feature flag
function getEligibleMarkets(game) {
...
function setNewTriggerEngine(enabled) {
...
function isMarketEligible(game, marketLabel) {
    const fn = exports.TRIGGER_MAP[marketLabel];
    if (!fn)
        return false;
    return fn(game);
}
